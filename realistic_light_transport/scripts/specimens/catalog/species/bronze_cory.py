...
def paint_fin(ctx):
    ...
    albedo = textures.rgba(FIN, 0.26, ctx.shape)
    ...
    if ctx.fin in ("dorsal1", "pectoral"):
        ...
        albedo = textures.mix(albedo, FIN_EDGE, leading_spine * 0.78)
    ...
    albedo[..., 3] = np.clip(0.26 - 0.14 * membrane_edge + 0.05 * ray + 0.56 * leading_spine, 0.0, 0.88)
