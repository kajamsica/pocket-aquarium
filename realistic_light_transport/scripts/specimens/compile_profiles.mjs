...
function compilePackage(packageDir) {
  ...
  if (acceptedHash !== manifest.promotion.acceptedHash || acceptance.runtimeGlbSha256.lod1 !== acceptedHash || acceptance.assetVersion !== manifest.revisions.asset) fail('accepted Ocellaris asset identity mismatch')
...
const first = render(firstCatalog)
const second = render(secondCatalog)
...
console.log(JSON.stringify({ status: 'passed', deterministic: true, output: relative(REPO_ROOT, OUTPUT), profiles: Object.keys(firstCatalog), acceptedHashes: Object.fromEntries(Object.entries(firstCatalog).map(([speciesId, profile]) => [speciesId, profile.acceptedAssetHash])) }))
