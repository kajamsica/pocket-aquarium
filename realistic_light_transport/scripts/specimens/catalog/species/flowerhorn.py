...
DEEP_RED = (0.48, 0.025, 0.018)
SCARLET = (0.82, 0.050, 0.020)
ORANGE_RED = (0.92, 0.14, 0.030)
PALE_GOLD = (0.84, 0.29, 0.085)
BELLY = (0.72, 0.13, 0.060)
FLOWER = (0.035, 0.025, 0.024)
FLOWER_EDGE = (0.18, 0.065, 0.035)
PEARL_BLUE = (0.18, 0.82, 0.94)
PEARL_WHITE = (0.80, 0.98, 1.00)
FIN_RED = (0.56, 0.035, 0.020)
...
def _pearl_mask(u, v, zeta):
    ...
    fine = paint.spots(u + warp_u, v + warp_v, density=49.0, radius=0.14, seed=73, jitter_radius=0.72)
    coarse = paint.spots(u - warp_u * 0.6, v + warp_v * 0.4, density=30.0, radius=0.17, seed=75, jitter_radius=0.65)
    clusters = smoothstep(0.38, 0.62, fbm(u * 17.0, v * 11.0, octaves=3, seed=79))
    sparse = smoothstep(0.51, 0.73, fbm(u * 31.0, v * 19.0, octaves=2, seed=81))
    lateral = 1.0 - smoothstep(0.76, 0.98, np.abs(zeta))
    head_keep = 1.0 - 0.24 * smoothstep(0.84, 0.98, u)
    varied = np.maximum(fine * clusters, coarse * sparse * 0.78)
...
def _flower_line(u, v, zeta):
    ...
    corridor = 1.0 - smoothstep(0.11, 0.31, np.abs(zeta - center))
    broad = smoothstep(0.40, 0.60, fbm(u * 13.0, zeta * 6.0, octaves=3, seed=47))
    narrow = smoothstep(0.45, 0.63, fbm(u * 25.0 + zeta * 3.0, zeta * 11.0, octaves=2, seed=51))
    breaks = smoothstep(0.45, 0.67, fbm(u * 8.0, zeta * 14.0, octaves=2, seed=53))
...
def paint_body(ctx):
    ...
    albedo = textures.mix(albedo, DEEP_RED, anterior * 0.30)
    albedo = textures.mix(albedo, ORANGE_RED, posterior * 0.30)
    albedo = textures.mix(albedo, BELLY, smoothstep(-0.28, -0.94, z) * 0.38)
    albedo = textures.mix(albedo, PALE_GOLD, posterior * smoothstep(-0.12, -0.82, z) * 0.20)
    albedo = textures.scale_rgb(albedo, 0.88 + 0.13 * grain)

    flowers = _flower_line(u, v, z)
    flower_edge = np.clip(3.2 * flowers * (1.0 - flowers), 0.0, 1.0)
    albedo = textures.mix(albedo, FLOWER_EDGE, flower_edge * 0.42)
    albedo = textures.mix(albedo, FLOWER, flowers * 0.96)

    pearls = _pearl_mask(u, v, z)
    pearl_glint = smoothstep(0.60, 0.92, fbm(u * 67.0, v * 31.0, octaves=2, seed=83))
    albedo = textures.mix(albedo, PEARL_BLUE, pearls * 0.88)
    albedo = textures.mix(albedo, PEARL_WHITE, pearls * pearl_glint * 0.76)
...
def paint_fin(ctx):
    ...
    albedo = textures.scale_rgb(albedo, 0.78 + 0.25 * rays)
    ...
    if ctx.fin in ("dorsal", "anal", "caudal"):
        ...
        albedo = textures.mix(albedo, PEARL_BLUE, pearls * 0.88)
        albedo = textures.mix(albedo, PEARL_WHITE, pearls * rays * 0.30)
        albedo = textures.mix(albedo, FIN_DARK, (1.0 - smoothstep(0.0, 0.12, ctx.V)) * 0.44)
        alpha = 0.91 - 0.18 * smoothstep(0.68, 1.0, ctx.V)
    else:
